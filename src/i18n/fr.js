// NOTE : les textes de chaque service (services[id]) ne sont pas encore
// traduits en français — ils restent absents et useIdioma() applique un
// repli vers le PT-PT, avec un avertissement dans la console. Tout le
// chrome de l'interface, les presets et les noms de couches sont traduits
// à 100 %. Voir le README pour le plan de traduction du contenu des 248
// services.
export default {
  meta: { code: 'fr', label: 'Français', dir: 'ltr' },
  ui: {
    title: "Carte de l'écosystème SAP",
    subtitle: "Le portefeuille SAP : ce qui existe, comment cela se connecte, et comment composer un paysage applicatif adapté au client.",
    searchPlaceholder: 'Rechercher (nom, acronyme, SCI, SAP for Me...)',
    searchEmpty: "Aucun service trouvé. Essayez l'acronyme (SCI, BTP, ALM) ou le nom commercial (SAP for Me).",
    showLegacy: 'Afficher le legacy',
    language: 'Langue',
    layers: 'Couches',
    edgeNative: 'Native',
    edgeCleanCore: 'Clean core (BTP)',
    edgeNetwork: 'Réseau',
    edgeGov: 'Gouverne',
    stateActive: 'Actif',
    stateRecommended: 'Recommandé',
    stateOptional: 'Optionnel',
    stateLegacy: 'Legacy',
    stateIrrelevant: 'Non pertinent',
    drawerWhat: 'Ce que ça fait',
    drawerUsedFor: 'À quoi ça sert',
    drawerMix: "Comment ça s'intègre",
    drawerExample: 'Exemple réel',
    drawerRelated: 'En relation avec',
    drawerDontConfuse: 'À ne pas confondre avec',
    drawerSatellites: "Satellites dans l'index Help",
    drawerFeatures: 'Fonctionnalités de ce produit : help.sap.com et Feature Documentation.',
    drawerScenario: 'Couche, type et état dans ce scénario',
    resetView: 'Réinitialiser la vue',
    selectedEdges: 'connexions',
    hiddenEdgesSuffix: 'connexions dans le panneau',
    clearSelection: 'Effacer la sélection',
    spineOnly: 'Uniquement la colonne du scénario',
    zoomIn: 'Zoomer',
    zoomOut: 'Dézoomer',
    closeDrawer: 'Fermer',
    back: 'Retour',
    viewMap: 'Carte',
    viewSchema: 'Schéma',
    esquemaFocusLegend: 'Composants au centre de ce scénario',
    esquemaInstrucoes: 'Cliquez sur un bloc pour ouvrir sa fiche. Les flèches en mouvement montrent le flux de ce scénario.',
    esquemaExplorarFluxo: 'Explorer le flux',
    esquemaPausarFluxo: 'Mettre le flux en pause',
    esquemaVerPortefolio: 'Voir le portefeuille',
    esquemaRodape: 'Flèches en mouvement = flux de ce scénario. Cliquez sur un bloc pour le détail. Carte = portefeuille complet.',
    northStarTitulo: 'North Star — quatre couches',
    viewNeeds: 'Besoins',
    necSituacao: 'Votre situation',
    necSituacaoAjuda: 'Choisissez ce qui ressemble le plus à votre entreprise. Ajustez ensuite.',
    necCapacidades: 'Ce que vous devez faire',
    necCapacidadesAjuda: 'En langage métier, pas en langage produit.',
    necLandscape: 'Ce que vous devez contracter',
    necTotalPecas: '{n} pièces',
    necVia: 'Voie contractuelle :',
    necVia_cloud: 'GROW (cloud public)',
    necVia_rise: 'RISE (privé)',
    necVia_onprem: 'On-premise',
    necPessoas: 'Personnes :',
    necJaTemSap: 'A déjà SAP',
    necMuitoCodigo: 'avec beaucoup de code sur mesure',
    necClasse_base: 'socle',
    necClasse_nucleo: 'essentiel',
    necClasse_conformidade: 'obligatoire au PT',
    necClasse_apoio: 'choisi',
    necClasse_dependencia: 'dépendance',
    necClasse_extra: 'transformation',
    necPerguntas: 'Questions qui changent la recommandation',
    necPerguntasAjuda: 'N’activez que ce qui est vrai chez vous.',
    necDecisoes: 'Décisions et avertissements honnêtes',
    necEmpregados: 'Ce qu’il faut donner à vos employés',
    necEmpregadosAjuda: 'Par métier : les applications que chacun ouvre au quotidien.',
    necAppsBloqueadas: 'Seulement avec plus de pièces',
    necRequer: 'nécessite {s} dans le paysage',
    necRodape: 'Noms d’applications Fiori standard de S/4HANA Cloud. Confirmez l’app ID et le business role exacts dans la SAP Fiori Apps Reference Library pour la version contractée.',
    viewBuilder: 'Constructeur',
    builderTitle: 'Paysage en construction',
    builderSearch: 'Chercher une pièce...',
    builderAll: 'Toutes',
    builderDragHint: 'Glissez sur la bonne couche, ou double-cliquez pour placer.',
    builderAdd: 'Ajouter',
    builderRemove: 'Retirer',
    builderPaletteEmpty: 'Rien trouvé dans cette couche.',
    builderPieces: 'pièces',
    builderLinks: 'liens',
    builderLayers: 'couches',
    builderTemplates: 'Architectures de référence',
    builderTemplatesHint: 'Partez d\u2019une architecture publiée par SAP, puis modifiez-la.',
    builderExport: 'Exporter',
    builderClear: 'Effacer',
    builderWrongLayer: 'Mauvaise couche — cette pièce est',
    builderEmptyLayer: 'vide',
    builderSuggestions: 'Suggestions',
    builderSuggestionsHint: 'Classées selon le nombre de diagrammes officiels SAP qui les dessinent près des vôtres.',
    builderInDiagrams: 'diagrammes SAP',
    builderNoSuggestions: 'Placez une pièce pour obtenir des suggestions.',
    builderChecks: 'Vérifications',
    builderAllGood: 'Aucun problème détecté.',
    builderConnections: 'Liens de cette pièce',
    builderSearch: 'Chercher un élément à ajouter',
    builderAllLayers: 'Toutes',
    builderAdd: 'Ajouter',
    builderRemove: 'Retirer',
    builderClear: 'Tout effacer',
    builderPieces: 'éléments',
    builderDropHere: 'déposez ici un élément de cette couche',
    builderEmpty: 'Faites glisser des éléments de la palette vers les couches, ou chargez un mélange type pour partir d\u2019un paysage déjà assemblé.',
    builderTemplates: 'Partir d\u2019un mélange type',
    builderValidator: 'Validation',
    builderSuggestions: 'Éléments manquants',
    builderAllGood: 'Paysage cohérent : chaque élément existe dans ce scénario et repose sur quelque chose.',
    builderNotInScenario: 'hors de ce scénario',
    ruleScenario: 'n\u2019existe pas dans ce scénario. Changez de preset ou retirez l\u2019élément.',
    ruleNoInfra: 'n\u2019a aucune infrastructure en dessous. Ajoutez un élément L0.',
    ruleNoFoundation: 'ne repose sur rien — aucune des couches dont il dépend n\u2019a été placée.',
    ruleLegacy: 'est un élément legacy en phase-out dans ce scénario — légitime pendant la transition, mais avec une date de fin.',
    ruleIsolated: 'n\u2019est relié à aucun autre élément de votre paysage.',
    builderDisclaimer: 'La validation provient du catalogue lui-même : couche (ce qui repose sur quoi), scénarios (si l\u2019élément existe dans ce preset) et ligaA (dépendances). Ce n\u2019est pas un configurateur de licences SAP.',
    profiles: {
      greenfield: 'Mid-market greenfield',
      brownfield: 'Brownfield ECC complexe',
      industria: 'Industrie / atelier de production',
      regulado: 'Régulé / souveraineté des données',
      spend: 'Fort volume d\u2019achats / réseau fournisseurs',
      workforce: 'Forte main-d\u2019œuvre externe',
    },
    infraSelector: { aws: 'AWS', azure: 'Azure', gcp: 'GCP', sci: 'SCI' },
    footerDisclaimer: "Contenu pédagogique fondé sur les offres publiques SAP et l'index help.sap.com/docs/all-products. Cas résumés à partir d'histoires publiques. Deploy with Confidence est une méthodologie d'ingénierie interne à SAP, pas une référence commerciale (SKU). L'index All Products compte environ 1 248 entrées documentaires ; cette carte montre les pièces d'architecture.",
  },
  presets: {
    onprem: {
      title: 'On-Premise traditionnel',
      hero: "Le client exploite le data center, NetWeaver, HANA, ECC ou S/4 any-premise, PI/PO et Solution Manager. Contrôle total, TCO et montées de version à la charge de l'IT. Des LoB cloud (SuccessFactors, Ariba, Concur) peuvent déjà exister comme des îlots — un hybride accidentel. Trajectoire de sortie : RISE ou GROW, avec PI/PO et SolMan en phase-out d'ici 2027.",
    },
    cloud: {
      title: 'Cloud public — GROW',
      hero: "S/4HANA Cloud Public Edition chez l'hyperscaler, processus standards, BTP pour les extensions, Integration Suite comme iPaaS, LoB SaaS à la carte, Cloud ALM inclus. DRC et Multi-Bank font partie du socle Finance honnête. Signavio pilote le fit-to-standard. Deploy with Confidence, côté SAP, permet à la Public Edition et à Signavio de recevoir l'innovation en cycle court ; côté client, cela se traduit par l'absence de personnalisation du core.",
    },
    rise: {
      title: 'Hybride — RISE with SAP',
      hero: "Un seul contrat : logiciel + BTP + infrastructure gérée par SAP chez l'hyperscaler (ou CDC). Le S/4 Private Edition \u2248 un périmètre on-premise avec exploitation SAP. Signavio décide du processus, LeanIX du landscape, BTC des données, Tricentis de la non-régression, Cloud ALM exécute et mesure le clean core, et Deploy with Confidence est la discipline pour déployer le to-be sans casser la prochaine montée de version. Les systèmes on-premise résiduels se connectent via Cloud Connector / Edge Integration Cell. PI/PO et SolMan en phase-out d'ici 2027. Les groupes multi-ECC peuvent utiliser Central Finance comme étape intermédiaire.",
    },
  },
  layers: {
    infra: 'Infrastructure',
    dados: 'Données',
    plataforma: 'Plateforme (SAP BTP)',
    integracao: 'Intégration',
    core: 'Core ERP / Digital Core',
    lob: 'LoB / SaaS / Industrie / Expérience / IA',
    alm: 'Cycle de vie, architecture et transformation',
  },
  services: {},
}

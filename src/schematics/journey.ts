import { floorTitle, growInternetCard, l0CardsFor, type L0Card } from "@/data/l0";
...
export function buildJourney(input: JourneyInput): Schematic {
  const { lang, preset, infra, template, caminhoMinimo, profiles, focus } = input;
  ...
  const topY = 72;
  const boxH = 400;
  const floorY = topY + boxH + 20;
  const floorH = focus === "floor" || !caminhoMinimo ? 280 : 200;
  ...
  // L0 floor
  let l0list: L0Card[] = l0CardsFor(preset, infra, caminhoMinimo && focus !== "floor", regulado);
  if (preset === "cloud") {
    l0list = [growInternetCard(), ...l0list.filter((c) => c.grow !== false && !c.id.includes("spoke"))];
  }
  const innerW = canvasW - 64;
  const cols = Math.min(Math.max(l0list.length, 1), 4);
  const cardW = Math.floor((innerW - (cols - 1) * 10) / cols);
  l0list.forEach((c, i) => {
    const col = i % cols;
  ...
  const titleMap = loc(
    `Do portátil do comercial até ao chão — ${mixName}`,
    `From the sales laptop to the floor — ${mixName}`,
    `Du portable du commercial jusqu'au sol — ${mixName}`,
    `Vom Laptop des Vertriebs bis zum Boden — ${mixName}`,
    `Del portátil del comercial hasta el suelo — ${mixName}`,
  );
  const subtitleMap = loc(
    "Cada caixa é uma coisa que alguém usa ou um sítio onde isso corre. As setas numeradas são a ordem. O chão (L0) é o fim.",
    "Each box is something a person uses or a place where it runs. Numbered arrows are the order. The floor (L0) is the end.",
    "Chaque boîte est une chose que quelqu'un utilise ou un endroit où ça tourne. Les flèches numérotées sont l'ordre. Le sol (L0) est la fin.",
    "Jede Kiste ist etwas, das jemand nutzt, oder ein Ort, an dem es läuft. Nummerierte Pfeile sind die Reihenfolge. Der Boden (L0) ist das Ende.",
    "Cada caja es una cosa que alguien usa o un sitio donde eso corre. Las flechas numeradas son el orden. El suelo (L0) es el fin.",
  );
  const foot = loc(
